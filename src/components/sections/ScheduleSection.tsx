import { Link, Typography, Accordion, AccordionDetails, AccordionSummary, Collapse, CircularProgress } from "@mui/material";
...
const ScheduleSection = (): ReactElement => {
  ...
  const getDateTime = (timestamp: number, useEST: boolean): { [key: string]: number } => {
    const dateTime = useEST
      ? new Date(new Date(timestamp / 1000).toLocaleString("en-US", { timeZone: "America/New_York" }))
  ...
  const getEventObjects = (events: Array<ScheduleItem>, idx: number): ReactElement => {
    ...
    events.forEach((event: ScheduleItem) => {
      ...
      const timeStart =
        ((hoursStart % 12 == 0 ? 12 : hoursStart % 12).toString()
          + ":"
          + (minutesStart > 9 ? minutesStart.toString() : "0" + minutesStart.toString())
          + (hoursStart >= 12 ? " pm" : " am"))
      ...
      if (dateStart == dateEnd && monthStart == monthStart) {
        dateTimeString = event.startTime == event.endTime
          ? timeStart + " EST"
          : timeStart + " - " + timeEnd + " EST"
      } else {
        dateTimeString = dateStart + " " + monthStart
          + " " + timeStart + " - \n"
          + dateEnd + " " + monthEnd
          + " " + timeEnd + " EST"
      }
      ...
      eventRecords.push(
        <Event key={eventRecords.length}>
          ...
          <EventDetails>
            ...
            <EventPlatform>
              {
                platformsStrings[event.platform]
              }
              {
                event.platformUrl
                  ? <StyledLink
    ...
    return (
      <EventGroup key={idx} defaultExpanded={dayToday == dayHeader && monthToday == monthHeader}
        sx={{
          '& .MuiCollapse-root': {
            width: '100%',
          },
        ...
        <EventGroupSummary expandIcon={<ExpandMoreIcon />}>
          <EventDate>
            {
              dayNames[dayOfWeekHeader] + ", " + dayHeader.toString() + " " + monthNames[monthHeader]
            }
          </EventDate>
        </EventGroupSummary>
        <EventGroupDetails>
  ...
  useEffect(() => {
    const getEvents = async () => {
      setLoading(true)
      try {
        const eventsUngrouped = await (await fetch(process.env.BACKEND_URL + "/schedule/")).json()
        ...
        eventsUngrouped.forEach((element: ScheduleItem) => {
          ...
          const key = (dateTimeStart["month"] * 32 + dateTimeStart["day"]).toString()
          if (groups[key]) {
